...
Notebook Capytale pour les exercices 1 et 2 : <button type="button" class="btn btn-light">[Capytale](https://capytale2.ac-paris.fr/web/c/0681-1609936){target="_blank"} </button>

:::

## {{< fa desktop >}} Exercice 1

Dans cet exercice, on utilise le fichier [CSV listant les établissements scolaires du Cantal](educ_cantal.csv) déjà rencontré dans le cours (Source : https://www.data.gouv.fr/fr/). Ce fichier utilise l'encodage UTF-8.
...
3. Opérations de **sélection** :
    ...
    3. En utilisant une boucle, écrire des instructions permettant de définir le tableau ``etab_AURILLAC`` contenant les enregistrements qui correspondent à des établissements situés à Aurillac.
...
4. Opérations de **tri** :
    ...
    1. Trier les données par ordre croissant de codes postaux.
    2. Trier les données d'ouest en est puis du nord au sud.
    3. Trier les données suivant deux critères : d'abord la commune, puis à l'intérieur de chaque commune, selon le statut : public en premier, privé en dernier.

5. Opération de **jointure** :

    1. Importer le fichier CSV [``population_Cantal.csv``](population_Cantal.csv) et indexer les données sous la forme d'un tableau de dictionnaires nommé ``population`` (Source des données : Insee, Recensement de la population 2019). Ce fichier utilise l'encodage UTF-8. 
...
Pour s'entraîner, on reprend le fichier [``population_Cantal.csv``](population_Cantal.csv). Pour chaque question, écrire des instructions Python permettant d'y répondre. Vous pourrez dans certains cas proposer plusieurs solutions.
