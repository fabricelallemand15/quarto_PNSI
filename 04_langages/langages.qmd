...
::: {.callout-tip}
## Travail à faire par groupes de deux

1. Choisissez un des treize langages de programmation en tête du classement TIOBE ci-dessous (chaque groupe doit choisir un langage différent !).
2. **En utilisant les informations de cette page**, que vous pouvez compléter par des recherches personnelles, préparez un court exposé sur le langage choisi : 
    * Présentation générale du langage (historique, utilisation, etc.)
    * Points forts et faiblesses
    * Exemples de code + comparaison avec Python
3. Présentez votre exposé à la classe (5 min).
:::
